...
- If `recapper` is run again for the same active session or workstream, prefer updating the existing recap note instead of creating a second recap note.
...
When `recapper` is invoked again after more user interaction, treat the existing recap note as the baseline when it appears to describe the same session or workstream.
